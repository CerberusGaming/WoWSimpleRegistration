...
| DISCORD_EMBED          | discord_embed             | ''                                                      | The Embed / Widget from the Discord Server Settings. This should start with `<iframe`. |
...
| SMTP_MAIL              | smtp_mail                 | `no-reply@example.com`                                  | This also controls the contact email.                                                  |
...
WoWSimpleRegistration has a language overide system. You can leverage it using the `LANG_*` variable.

Example:

```shell
LANG_RULE: "Rool" # Environments are all caps, and language is all lowercase. The code takes care of this conversion.
```

## Rules

You can add rules via the `RULE_*` variable.

Example:

```shell
RULE_1: "No Hacking."
RULE_2: "No Abusive Language."

>> generates <<

<li>Rule 1. No Hacking.</li>
<li>Rule 2. No Abusive Language.</li>
```

## How To Connect

You can generate lines in the `How To Connect` page via `HOWTO_*`.
